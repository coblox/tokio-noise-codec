...
impl<C: Decoder> NoiseCodec<C> {
    fn decrypt<T: From<Vec<u8>>>(
        ...
        let plain_text_length = number_of_bytes_to_decrypt - NOISE_TAG_LENGTH;

        let mut plain_text = vec![0; plain_text_length];

        let cipher_text = cipher_text.split_to(number_of_bytes_to_decrypt);
        self.noise.read_message(&cipher_text[..], &mut plain_text)?;
